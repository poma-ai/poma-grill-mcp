...
| `invalid_input` | Bad arguments — e.g. zero or two of `file_path`/`file_base64`/`url`, a path outside `GRILL_INGEST_ALLOWED_PREFIX`, `file_path` on the hosted HTTP server (no `GRILL_INGEST_ALLOWED_PREFIX` set), a payload over `GRILL_INGEST_MAX_BYTES`, or a `project_id`/`POMA_PROJECT_ID` that differs from the project the project API key is bound to (API `409 project_id_conflict`). | Fix the arguments. Never retry unchanged. |
